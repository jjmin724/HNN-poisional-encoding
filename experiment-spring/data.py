...
#             # GPU version
#             # t_embedded = timestep_embedding(torch.tensor(t), 16).detach().cpu().numpy()
#             t_embedded = timestep_embedding(t, 16)
#             tmp_xy = np.stack([x, y])
#             t_embedded = t_embedded.astype(tmp_xy.dtype, copy=False)

#             xs.append( np.vstack([tmp_xy, t_embedded.T]).T )
