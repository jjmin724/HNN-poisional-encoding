...
#     half = dim // 2
#     log_max_period = np.log(np.array([max_period]))[0]
#     freqs = torch.exp(
#         -log_max_period * torch.arange(start=0, end=half, dtype=torch.float32) / half
#     ).to(device=t.device)
...
def timestep_embedding(t, dim, max_period=10000.0):
    """
    Create sinusoidal timestep embeddings (NumPy/autograd.numpy version).
    t: 1-D array-like of length T
    return: (T, dim) array
    """
    t = np.array(t)  # autograd.numpy 배열
    half = dim // 2
