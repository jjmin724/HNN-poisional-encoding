...
class HNN(torch.nn.Module):
    ...
    def __init__(self, input_dim, differentiable_model, field_type='solenoidal',
                    baseline=False, assume_canonical_coords=True, time_series=False):
    ...
    def time_derivative(self, x, t=None, separate_fields=False):
        ...
        conservative_field = torch.zeros_like(x[:, :2]) # start out with both components set to 0
        solenoidal_field = torch.zeros_like(x[:, :2])
        ...
        if self.field_type != 'solenoidal':
            ...
            print(conservative_field.shape)
            conservative_field = conservative_field[:, :2]
            print(conservative_field.shape)
        ...
        if self.field_type != 'conservative':
            ...
            solenoidal_field = solenoidal_field[:, :2]
            print(solenoidal_field.shape)
